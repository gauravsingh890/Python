...
def send_email(sender, receiver, recipients):
    password = os.environ['password']
    body_msg = f'''\
        From: {sender}
        Subject: Your Secret Santa Present

        Hi! Your secret santa is: {recipients}!
        Remember to send 10$-20$ on your gift, but don't stress about it being the perfect
        '''
...
while len(names_list) >= 2:
    send_email('apnamaillikho', names_and_emails[0][1], names_and_emails[1][0])
    names_and_emails.pop(0)
    random.shuffle(names_and_emails)

send_email('bolanaapnamaillikho', names_and_emails[0][1],first_name)